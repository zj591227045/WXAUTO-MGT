...
class TestConfigManager(IsolatedAsyncioTestCase):
    ...
    async def test_export_import_config(self):
        ...
        # 修改配置
        self.config_manager.set('app.name', "旧名称")
    ...
    def test_load_from_env(self):
        ...
        # 从环境变量加载
        env_config = self.config_manager.load_from_env()
        
        # 验证配置
        self.assertEqual(self.config_manager.get('app.name'), "环境变量名称")
        self.assertEqual(self.config_manager.get('app.debug'), True)
        self.assertEqual(self.config_manager.get('message_listener.max_listeners'), 50)
        
        # 清理环境变量
        del os.environ['WXAUTO_APP_NAME']
        del os.environ['WXAUTO_APP_DEBUG']
        del os.environ['WXAUTO_MESSAGE_LISTENER_MAX_LISTENERS']
